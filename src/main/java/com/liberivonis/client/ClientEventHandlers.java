...
public final class ClientEventHandlers {
    public static final KeyMapping OPEN_HUB = new KeyMapping("key.liber_ivonis.open_hub", InputConstants.Type.KEYSYM,
            GLFW.GLFW_KEY_UNKNOWN, "key.categories.liber_ivonis");
    ...
    public static void clientSetup(net.neoforged.fml.event.lifecycle.FMLClientSetupEvent event) {
        if (ModList.get().isLoaded("ftblibrary"))
            event.enqueueWork(com.liberivonis.compat.FtbLibraryCompat::init);
    }

    @SubscribeEvent
    public static void onClientTick(net.neoforged.neoforge.client.event.ClientTickEvent.Post event) {
        Minecraft mc = Minecraft.getInstance();
        while (OPEN_HUB.consumeClick() && mc.player != null && mc.screen == null)
            ClientHooks.openHub();
    }

    @SubscribeEvent
    public static void onMouseClick(InputEvent.InteractionKeyMappingTriggered event) {
        if (event.getKeyMapping() != Minecraft.getInstance().options.keyUse || Minecraft.getInstance().player == null)
            return;
