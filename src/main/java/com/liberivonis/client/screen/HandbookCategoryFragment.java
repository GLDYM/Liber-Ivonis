...
public final class HandbookCategoryFragment extends Fragment {
    ...
    public HandbookCategoryFragment(Component title, List<HandbookEntry> entries) {
        this.title = title;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, icyllis.modernui.util.DataSet state) {
        ...
        for (int i = 0; i < entries.size(); i++) {
            if ((i & 1) == 0) {
                ...
                list.addView(row,
                        params(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT, 0, 0, 0, 8));
            }
            HandbookEntry entry = entries.get(i);
    ...
    private static LinearLayout.LayoutParams params(int w, int h, int l, int t, int r, int b) {
